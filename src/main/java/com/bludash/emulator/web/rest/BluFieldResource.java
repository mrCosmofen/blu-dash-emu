...
/**
 * REST controller for managing {@link com.bludash.emulator.domain.BluField}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BluFieldResource {
    ...
    public List<BluField> getAllBluFields(@RequestParam(required = false) String filter) {
        if ("blufieldstringvalue-is-null".equals(filter)) {
            log.debug("REST request to get all BluFields where bluFieldStringValue is null");
            return StreamSupport
                .stream(bluFieldRepository.findAll().spliterator(), false)
                .filter(bluField -> bluField.getBluFieldStringValue() == null)
                .collect(Collectors.toList());
        }
        if ("blufieldcurrencyvalue-is-null".equals(filter)) {
            log.debug("REST request to get all BluFields where bluFieldCurrencyValue is null");
            return StreamSupport
                .stream(bluFieldRepository.findAll().spliterator(), false)
                .filter(bluField -> bluField.getBluFieldCurrencyValue() == null)
                .collect(Collectors.toList());
        }
        if ("blufieldnumbervalue-is-null".equals(filter)) {
            log.debug("REST request to get all BluFields where bluFieldNumberValue is null");
            return StreamSupport
                .stream(bluFieldRepository.findAll().spliterator(), false)
                .filter(bluField -> bluField.getBluFieldNumberValue() == null)
                .collect(Collectors.toList());
        }
