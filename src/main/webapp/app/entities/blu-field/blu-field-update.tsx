...
import { getEntities as getBluFieldStringValues } from 'app/entities/blu-field-string-value/blu-field-string-value.reducer';
import { IBluFieldCurrencyValue } from 'app/shared/model/blu-field-currency-value.model';
import { getEntities as getBluFieldCurrencyValues } from 'app/entities/blu-field-currency-value/blu-field-currency-value.reducer';
import { IBluFieldNumberValue } from 'app/shared/model/blu-field-number-value.model';
import { getEntities as getBluFieldNumberValues } from 'app/entities/blu-field-number-value/blu-field-number-value.reducer';
...
export const BluFieldUpdate = (props: IBluFieldUpdateProps) => {
  const [bluFieldStringValueId, setBluFieldStringValueId] = useState('0');
  const [bluFieldCurrencyValueId, setBluFieldCurrencyValueId] = useState('0');
  const [bluFieldNumberValueId, setBluFieldNumberValueId] = useState('0');
  const [bluFormId, setBluFormId] = useState('0');
  const [isNew, setIsNew] = useState(!props.match.params || !props.match.params.id);

  const { bluFieldEntity, bluFieldStringValues, bluFieldCurrencyValues, bluFieldNumberValues, bluForms, loading, updating } = props;
  ...
  useEffect(() => {
    ...
    props.getBluFieldStringValues();
    props.getBluFieldCurrencyValues();
    props.getBluFieldNumberValues();
    props.getBluForms();
  }, []);
...
const mapStateToProps = (storeState: IRootState) => ({
  bluFieldStringValues: storeState.bluFieldStringValue.entities,
  bluFieldCurrencyValues: storeState.bluFieldCurrencyValue.entities,
  bluFieldNumberValues: storeState.bluFieldNumberValue.entities,
...
const mapDispatchToProps = {
  getBluFieldStringValues,
  getBluFieldCurrencyValues,
  getBluFieldNumberValues,
  getBluForms,
  getEntity,
  updateEntity,
  createEntity,
